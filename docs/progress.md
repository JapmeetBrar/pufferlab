...
| M1-A | [Narrow turbopuffer provider with fake and opt-in live tests](implementation-plan.md#task-t2) | provider_worker | `codex/t2-turbopuffer-provider` / [PR #5](https://github.com/JapmeetBrar/pufferlab/pull/5) | review_requested | Reviewer requested changes after finding retained secret-bearing SDK exception context and stale ledger state. Worker returned the task to `implementing`, detached mapped errors on every SDK path, added full-chain regression coverage, reran all gates, and requested re-review. Multi-query, server RRF, recall/warm hints, and live hybrid parity remain deferred to M1-C/T5. |
...
| 2026-08-22 | M1-A / [PR #5](https://github.com/JapmeetBrar/pufferlab/pull/5) | `review_requested → changes_requested → implementing → review_requested` | Review at `7ec8221` proved that mapped errors retained secret-bearing SDK exceptions through `__context__` and found a stale ledger. The worker now raises safe errors outside the active handler and tests `__context__`, `__cause__`, representations, and fully formatted chained tracebacks across query, write, metadata, delete, and close; full local gates pass and re-review is requested. |
