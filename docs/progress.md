...
| M1-C | [Retrieval orchestration](implementation-plan.md#task-t5) and [compare API](implementation-plan.md#task-t8) | provider_worker | `codex/t5-compare-api` / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | review_requested | Initial review found missing schema-bound filter validation; first re-review verified the core repair but found that the production runtime checked credentials before invoking it. The second repair stores the compiled write spec/validator in `RuntimeSearchBackend` and validates before credential checks, provider/embedder construction, or service execution, while retaining service-level defense-in-depth. Full backend, frontend, generated-contract, secret, vector, and cast gates pass; independent re-review is required. Hybrid multi-query/server RRF, warm hints, eval execution, and the browser UI remain deferred. |
