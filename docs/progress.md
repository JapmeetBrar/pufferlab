...
| M1-F | [Live vertical-slice verification](live-verification.md) | root + reviewer | `codex/m1-live-verification` / [draft PR #9](https://github.com/JapmeetBrar/pufferlab/pull/9) | implementing | [Early dedicated review](https://github.com/JapmeetBrar/pufferlab/pull/9#issuecomment-5381764970) reproduced a skipped-success credential bug and found API, secret-scan, evidence, cleanup, and self-referential-ledger blockers. [Re-review at `d98b25a`](https://github.com/JapmeetBrar/pufferlab/pull/9#issuecomment-5382354782) closed four classes and requested stricter API evidence plus exit/signal status propagation. Repair is active. The checked-in real-model verifier loads the exact BGE revision through production query/passage boundaries and reports finite, 384-dimensional, unit-normalized vectors without values. Credentialed provider, ingestion/rerun, browser, cleanup, final re-review, and protected merge remain pending. |
...
| 2026-08-22 | M1-F / [draft PR #9](https://github.com/JapmeetBrar/pufferlab/pull/9) | `implementing → review_requested → changes_requested → implementing → review_requested → changes_requested → implementing` | [Early review at `bced42b`](https://github.com/JapmeetBrar/pufferlab/pull/9#issuecomment-5381764970) kept the PR draft after reproducing the opt-in live test's `.env` skipped-success path and finding missing executable API/secret evidence, inaccurate CLI claims, non-mandatory cleanup, unsupported real-model evidence, and future self-merge fields. Head `d98b25a` added fail-closed `.env` loading, exact real-model/API/secret verifiers, explicit observed-readiness output, an immutable ignored namespace session, and a non-self-referential finalization handoff. [Re-review](https://github.com/JapmeetBrar/pufferlab/pull/9#issuecomment-5382354782) verified four repaired blocker classes and found that the API verifier did not bind result IDs or reject weak score/timing evidence and that cleanup traps did not preserve exit/signal failure. The next repair binds all evidence and propagates cleanup status across Bash and Zsh. Live credentials have not yet been consumed. |
