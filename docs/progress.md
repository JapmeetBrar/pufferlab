...
| 2026-08-22 | M1-E / `codex/m1-ingest-cli` | `implementing → review_requested` | The worker wired the checked-in 20-document fixture to the exact pinned BGE passage embedder and merged ingestion/provider services behind one injectable CLI command. Tests cover help/preflight behavior, owned namespace generation and validation, exact manifest/model/revision, progress and success output, same-namespace idempotence, sanitized failures and exit codes, and the absence of deletion. Local gates pass with 151 backend tests, one expected live skip, and one frontend test; live credential execution remains pending. |
