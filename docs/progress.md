...
| M1-C | [Retrieval orchestration](implementation-plan.md#task-t5) and [compare API](implementation-plan.md#task-t8) | provider_worker | `codex/t5-compare-api` / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | review_requested | Initial review found that filter overrides reached the provider without schema-bound operator/value validation. The repair validates the complete AST against the compiled fixture schema before any provider call or embedding, rejects unknown/non-filterable fields, invalid scalar/array shapes, type mismatches, and recursively non-finite JSON, and returns detached direct 422 errors. Full backend, frontend, generated-contract, secret, vector, and cast gates pass; independent re-review is required. Hybrid multi-query/server RRF, warm hints, eval execution, and the browser UI remain deferred. |
...
| 2026-08-22 | M1-C / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | `implementing → review_requested → changes_requested → implementing → review_requested` | The worker normally merged M1-B from protected `main`, integrated the exact manifest and compiled schema hash without casts, and added the production-shaped BM25/ANN compare service and API. [Initial review](https://github.com/JapmeetBrar/pufferlab/pull/6#issuecomment-5379587091) verified the live SDK request/response shapes but found that filter overrides lacked schema-bound validation and could turn invalid caller input into a provider error or non-finite wire value. The repair validates fields, filterability, operator/value shape and type, logical nesting, and finite JSON before dependencies execute; focused service/API tests prove direct redacted 422 responses with zero provider calls and zero embedding. No live credential was consumed; the existing isolated provider live test remains opt-in. |
