...
| M1-C | [Retrieval orchestration](implementation-plan.md#task-t5) and [compare API](implementation-plan.md#task-t8) | provider_worker | `codex/t5-compare-api` / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | review_requested | The branch normally merged protected `main`, consumes the exact fixture manifest/schema types without casts, and adds deterministic BM25/ANN config discovery, pinned lazy query embedding, compare evidence, redacted errors, API/runtime wiring, OpenAPI/types, and 18 focused fake tests. Full backend gates pass with 106 tests and one opt-in live skip; frontend and generated-contract gates pass. Independent review is required. Hybrid multi-query/server RRF, warm hints, eval execution, and the browser UI remain deferred. |
...
| 2026-08-22 | M1-C / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | `implementing → review_requested` | The worker normally merged M1-B from protected `main`, integrated the exact manifest and compiled schema hash without casts, added the production-shaped BM25/ANN compare service and API, and requested independent review after 106 backend tests plus all static, generated-contract, and frontend gates passed. No live credential was consumed; the existing isolated provider live test remains opt-in. |
