...
| 2026-08-22 | M1-D / [PR #8](https://github.com/JapmeetBrar/pufferlab/pull/8) | `planned → assigned → implementing → review_requested` | The worker based the UI on merged compare API commit `5ca296c`, normally merged protected `main` at `a5442f9`, and used only generated OpenAPI types for browser domain data. The focused UI suite passes 6 tests, generated types are zero-diff, and production build/secret scans pass. Local desktop and 390 px browser checks verified seeded config defaults, keyboard-labelled controls, stable query/config URL restoration, direct API error/retry behavior, responsive stacking, and WCAG AA small-text contrast. Successful result rendering is fake-contract tested; live search remains a separate pending verification step. |
