...
| M1-C | [Retrieval orchestration](implementation-plan.md#task-t5) and [compare API](implementation-plan.md#task-t8) | provider_worker | `codex/t5-compare-api` / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | verified | Dedicated review approved the production-order validation repair; PR #6 merged to protected `main` as `5ca296c` with Backend and Frontend checks green. Hybrid multi-query/server RRF, warm hints, eval execution, and the browser UI remain deferred. |
| M1-D | [Playground query UI with side-by-side results](implementation-plan.md#task-t8) | frontend_worker | `codex/t8-playground` / [PR #8](https://github.com/JapmeetBrar/pufferlab/pull/8) | review_requested | Generated OpenAPI types exclusively drive config discovery, the compare request, typed scores, timings, overlap, and rank evidence. Six browser-component tests cover config load, successful BM25/vector evidence, stable URLs, connecting/loading/disabled, empty, direct API error, and both retry paths. Frozen frontend, generated-contract, full backend, secret scan, production build, and desktop/mobile local browser gates pass; independent review is required. |
...
| 2026-08-22 | M1-C / [PR #6](https://github.com/JapmeetBrar/pufferlab/pull/6) | `implementing → review_requested → changes_requested → implementing → review_requested → changes_requested → implementing → review_requested → approved → merged → verified` | The worker normally merged M1-B from protected `main`, integrated the exact manifest and compiled schema hash without casts, and added the production-shaped BM25/ANN compare service and API. [Initial review](https://github.com/JapmeetBrar/pufferlab/pull/6#issuecomment-5379587091) found that filter overrides lacked schema-bound validation. The first repair added contract and service validation; [first re-review](https://github.com/JapmeetBrar/pufferlab/pull/6#issuecomment-5379634560) verified it but found that `RuntimeSearchBackend` checked missing credentials before constructing the validated service. The second repair validates with the same stored compiled write spec before `_get_service`; actual runtime/API tests prove invalid filters return detached 422 errors with missing credentials and zero provider/embedder factory calls, while a valid filter still returns the expected configuration 503. Dedicated re-review approved it; PR #6 merged as `5ca296c` with protected Backend and Frontend checks green. No live credential was consumed; the isolated provider live test remains opt-in. |
| 2026-08-22 | M1-D / [PR #8](https://github.com/JapmeetBrar/pufferlab/pull/8) | `planned → assigned → implementing → review_requested` | The worker based the UI on merged compare API commit `5ca296c` and used only generated OpenAPI types for browser domain data. The focused UI suite passes 6 tests, the full backend passes 136 tests with the opt-in live test skipped, generated types are zero-diff, and production build/secret scans pass. Local desktop and 390 px browser checks verified seeded config defaults, keyboard-labelled controls, stable query/config URL restoration, direct API error/retry behavior, and responsive stacking. Successful result rendering is fake-contract tested; live search remains explicitly assigned to M1-E. |
