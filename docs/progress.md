...
| M1-A | [Narrow turbopuffer provider with fake and opt-in live tests](implementation-plan.md#task-t2) | provider_worker | `codex/t2-turbopuffer-provider` / [PR #5](https://github.com/JapmeetBrar/pufferlab/pull/5) | review_requested | First re-review verified detached provider errors and the corrected ledger, then requested `k3` contract alignment. Worker added frozen `k3` to the provider-neutral type, exact fake write coverage, and opt-in live schema; full local gates and GitHub run `32563116311` passed, so re-review is requested. Multi-query, server RRF, recall/warm hints, and live hybrid parity remain deferred to M1-C/T5. |
...
| 2026-08-22 | M1-A / [PR #5](https://github.com/JapmeetBrar/pufferlab/pull/5) | `review_requested → changes_requested → implementing → review_requested → changes_requested → implementing → review_requested` | Review at `7ec8221` found retained secret-bearing exception context and a stale ledger; the worker fixed both at `2baa4a1`. First re-review verified those fixes, then found that `FullTextSearchSchema` omitted frozen, provider-supported `k3`. Head `7c2c874` aligns the typed boundary and exact fake/live write shapes; full local gates and GitHub Backend/Frontend checks pass, and another review is requested. |
